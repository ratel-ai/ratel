...
- Initial release. Pure-TypeScript client for Ratel Cloud telemetry — no native addon, runs on edge
  ...
  - `sendBatch` — best-effort `fetch` batch POST with exponential backoff + jitter; never throws.
  - `RatelCloud` — non-blocking `record` / `flush` / `close`, batching, and a periodic auto-flush timer.
