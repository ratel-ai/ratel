...
then exercises the raw send_batch transport for a concrete accept result, and
...
from ratel_ai_cloud import RatelCloud, send_batch
...
async def main() -> None:
    ...
    result = await send_batch(valid, endpoint=batch_endpoint, api_key=api_key)

    # (c) genuine ingestion: a fresh, never-sent event must be newly ingested.
    probe = await send_batch([_unique_event("py")], endpoint=batch_endpoint, api_key=api_key)
