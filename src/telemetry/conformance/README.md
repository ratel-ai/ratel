...
`fixtures.json` is one canonical set of `(scenario -> expected emitted span name + attributes)`.
Each language helper's conformance test loads it, builds each span **from its own `ratel.*`
constants through the real OpenTelemetry SDK**, and asserts the emitted span matches
`expect_name` / `expect_attributes` exactly. A constant that drifts from the pinned convention
(`../CONVENTIONS.md`) fails here, and it fails identically in every language because the
expectations live in one file.
...
The logical ids in `span` / `set` are the constant names lowercased; each helper keeps a small
map from logical id to its own constant, and those maps are the unit under test.
