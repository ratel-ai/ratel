...
- A `"semantic"`/`"hybrid"` catalog embeds each tool/skill **eagerly at `register`** (incrementally), so searches never pay the corpus-embedding cost. New `catalog.warm()` pre-computes embeddings on demand (e.g. after a bulk register). BM25 catalogs do neither.
