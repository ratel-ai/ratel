...
- Retrieval is now **dense (semantic)** — `search()` ranks by embedding cosine similarity (`BAAI/bge-small-en-v1.5`) via the bundled `ratel-ai-core`. The `search()` / `searchWithOrigin()` API is unchanged, so upgrading is transparent, but `SearchHit.score` is now a cosine similarity and the engine downloads a small embedding model on first use (cached, ~130 MB). See the [`ratel-ai-core` changelog](../../core/lib/CHANGELOG.md) and [ADR-0013](../../../docs/adr/0013-dense-semantic-retrieval.md).
