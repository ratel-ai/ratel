...
- Retrieval is now **hybrid** — BM25 + dense embeddings fused with Reciprocal Rank Fusion — via the bundled `ratel-ai-core`. The `search()` / `searchWithOrigin()` API is unchanged, so upgrading is transparent, but `SearchHit.score` is now the RRF fusion score and the dense arm downloads a small embedding model (`bge-small-en-v1.5`) on first use (cached, ~130 MB). See the [`ratel-ai-core` changelog](../../core/lib/CHANGELOG.md) and [ADR-0013](../../../docs/adr/0013-hybrid-retrieval.md).
