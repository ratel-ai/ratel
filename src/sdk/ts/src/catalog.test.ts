...
describe("ToolCatalog observe (tokens saved)", () => {
  ...
  it("records tokens_saved and lastSavings when observing", () => {
    ...
    const events = catalog.drainTraceEvents() as Array<Record<string, unknown>>;
    const saved = events.find((e) => e.type === "tokens_saved");
    expect(saved).toBeDefined();
    expect(saved?.top_k).toBe(1);
    expect(Number(saved?.full_catalog_tokens)).toBeGreaterThan(Number(saved?.selected_tokens));
  });
