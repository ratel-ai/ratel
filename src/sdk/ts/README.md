...
Telemetry export is optional. With `@ratel-ai/telemetry-otlp` installed, `configureTelemetry()` reads `RATEL_OTLP_ENDPOINT` and `RATEL_API_KEY`, wires the exporter, and returns a shutdown handle. It exports only `gen_ai.*`/`ratel.*` signal spans by default — pass `exportAllSpans: true` to forward all application spans. Message/tool content stays off by default; opt in with `captureContent`/`includeSpanAndEvents` (see the [telemetry guide](https://docs.ratel.sh/docs/telemetry) for the capture modes and their privacy implications).
