...
Beyond shrinking the prompt, ship one *usage rollup* per agent interaction to Ratel's cloud — the exact shape the dashboard renders. A rollup carries token spend broken down by the five context sources (`skills, tools, history, memory, user_input`), plus what Ratel selection saved and what it *could* save. The token / savings / cost maths live in `ratel-ai-core` (native); this SDK assembles the rollup, and the [`@ratel-ai/cloud`](../../integrations/cloud) package ships it. Design: [ADR-0013](../../../docs/adr/0013-observability-and-analytics.md).

`@ratel-ai/sdk` owns the *rollup* and the export *seam*: `buildRollup(input)` turns one interaction into the wire shape, and `Transport` is the interface a shipper implements. The concrete, env-configured cloud client (`RatelClient`, `track`, `getClient`) lives in [`@ratel-ai/cloud`](../../integrations/cloud):

```ts
import { buildRollup } from "@ratel-ai/sdk";
...
// → POST it to `{host}/api/v1/events` yourself, or let @ratel-ai/cloud batch + ship it.
```

Exported from `@ratel-ai/sdk`: `buildRollup`, `CONTEXT_SOURCES`, the native `estimateTokens` / `estimateCostUsd`, and the types `TrackInput` / `SourceTokens` / `Rollup` / `ContextSource` / `PartialSources` / `Transport`. The batching client and its `RatelClientOptions` are in [`@ratel-ai/cloud`](../../integrations/cloud).
