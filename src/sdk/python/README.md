...
Use `ToolCatalog` for ranked tools with sync or async handlers and `SkillCatalog` for ranked Markdown playbooks loaded on demand. Expose `search_capabilities_tool`, `invoke_tool_tool`, and `get_skill_content_tool` so an agent can discover tools and skills, invoke tools, and load full skill instructions — including a skill's declared tool and skill dependencies, expandable per call via `maxDepth`. Tools from existing MCP servers can be ingested into the tool catalog with the `mcp` extra.
