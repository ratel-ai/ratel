...
Use `ToolCatalog` for ranked tools with sync or async handlers and `SkillCatalog` for ranked Markdown playbooks loaded on demand. Expose `search_capabilities_tool`, `invoke_tool_tool`, and `get_skill_content_tool` so an agent can discover tools and skills, invoke tools, and load full skill instructions. Tools from existing MCP servers can be ingested into the tool catalog with the `mcp` extra. For constant grounding content — a shop's address, a brand's voice — the **experimental**, opt-in `ratel_ai.experimental` namespace adds `FactCatalog` (`from ratel_ai.experimental import FactCatalog`): register facts always-on or retrieval-gated, then inject them in one of two modes — `ground()` persists facts into your history and re-injects only when one isn't already fresh in the transcript, while `ground_snapshot()` returns the full per-call set statelessly (no markers, nothing stored). This API may change or be removed without a major version bump.
