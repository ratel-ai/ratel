...
The Python SDK. Bundles `ratel-ai-core` via a [PyO3](https://pyo3.rs) native binding under [`python/native/`](python/native/README.md), distributed as prebuilt `abi3` wheels. Feature parity with the TS SDK on the catalog / gateway / skills surface, **plus a Python-only observability layer** — Langfuse-style tracing, drop-in provider wrappers, and transparent tool selection ([ADR 0013](../../docs/adr/0013-python-observability-layer.md)–[0015](../../docs/adr/0015-transparent-tool-selection.md)). See [`python/README.md`](python/README.md) for usage.
