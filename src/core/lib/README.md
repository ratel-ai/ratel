...
`Skill` is the second content type, ranked by the same BM25 engine through `SkillRegistry`. A skill is indexed over its `name`, `description`, and `tags`; its `body` is the dispatch payload and is not indexed. See [ADR‑0011](../../../docs/adr/0011-first-class-skills.md). The same retrieval primitive carries forward to memories and message history as those land on the [roadmap](../../../docs/roadmap.md).
