...
1. **Pre-filter** ([ADR-0004](../../docs/adr/0004-kernel-retrieval-and-tool-selection.md) `replace` mode) — the catalog is registered in a `ToolCatalog`; before the model call, BM25 narrows it to the top-K most relevant tools for the prompt. Those land directly in the agent's tool list with full schemas.
