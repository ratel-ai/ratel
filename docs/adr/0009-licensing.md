# 9. Licensing: Apache-2.0 engine, MIT everything else
...
The engine is the reusable asset others embed: for a library whose whole point is to ship
...
engine (FFI surfaces, `init()` sugar, examples) living in MIT-dominant ecosystems where
...
| Component | License |
|---|---|
| `ratel-ai-core` (the engine crate) | **Apache-2.0** |
| SDKs (`@ratel-ai/sdk`, `ratel-ai`, the native crates) | MIT |
| Telemetry helpers | MIT |
| Examples | MIT |

`LICENSE-APACHE` at the repo root carries the Apache text for the engine; `LICENSE.md`
...
- Downstream embedders of the engine get patent protection from its first Apache-2.0
...
- The dependency tree is mixed-license and that is fine: an MIT SDK over an Apache-2.0
  engine is a standard, one-way-compatible combination; nothing is more restrictive than the
