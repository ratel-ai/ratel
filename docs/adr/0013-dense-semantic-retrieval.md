...
- **API: additive.** New `search_dense` / `search_dense_with_origin` on both `ToolRegistry` and `SkillRegistry` mirror the BM25 `search` signature (`(query, top_k) -> Vec<…Hit>`) and emit the existing `Search` / `SkillSearch` trace event with a stage named `"dense"` — no trace-schema change ([ADR-0009](0009-trace-events-core-owned-schema.md)). `search()` is untouched. Surfaced as `searchDense` (TS) / `search_dense` (Python).
- **Scope: tools and skills, dense-only.** Dense ranking lands on both registries (the benchmark exercises tools via BFCL and skills via SR-Agents). A BM25+dense **hybrid** fusion is deliberately deferred; the `Embedder` trait leaves room for it.
