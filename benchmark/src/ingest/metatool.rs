...
/// one source of truth. Tracked alongside the upstream commit SHA in
/// `benchmark/test-data/SOURCES.md`.
...
/// Sampling spec for the produced corpus.
///
/// `multi_tool_ratio` is best-effort: if fewer multi-tool rows are available than
/// requested, the remainder is filled from single-tool rows up to `total`.
#[derive(Debug, Clone, Copy)]
pub struct SampleSpec {
    pub total: usize,
    pub multi_tool_ratio: f64,
    pub seed: u64,
}
...
/// Read MetaTool inputs, sample, and write a normalized JSONL to `output`.
pub fn ingest_to_jsonl(
    ...
    let (scenarios, mut stats) = build_scenarios(&single, &multi, &plugins, sample);
...
fn build_scenarios(
    ...
    // Filter unknown-gold queries first, so the sample draws from a known-valid
    // universe and IDs derived from row position stay stable.
    let mut single_valid: Vec<Scenario> = Vec::new();
    for q in single {
        match build_single_scenario(q, plugins) {
            Some(s) => single_valid.push(s),
            None => stats.skipped_unknown_gold += 1,
        }
    }
    let mut multi_valid: Vec<Scenario> = Vec::new();
    for q in multi {
        match build_multi_scenario(q, plugins) {
            Some(s) => multi_valid.push(s),
            None => stats.skipped_unknown_gold += 1,
        }
    }

    let want_multi = (spec.total as f64 * spec.multi_tool_ratio).round() as usize;
    let take_multi = want_multi.min(multi_valid.len());
    let want_single = spec.total.saturating_sub(take_multi);
    let take_single = want_single.min(single_valid.len());

    let mut rng = rand::rngs::StdRng::seed_from_u64(spec.seed);
    single_valid.shuffle(&mut rng);
    multi_valid.shuffle(&mut rng);

    let mut out: Vec<Scenario> = Vec::with_capacity(take_single + take_multi);
    out.extend(single_valid.into_iter().take(take_single));
    out.extend(multi_valid.into_iter().take(take_multi));

    // Stable output ordering by id, so the committed JSONL diff stays readable
...
mod tests {
    ...
    #[test]
    fn build_scenarios_sampling_is_deterministic_under_seed() {
        let plugins = fixture_plugins();
        let single: Vec<RawSingleQuery> = (0..50)
            ...
            .collect();
        let multi: Vec<RawMultiQuery> = (0..10)
            .map(|i| RawMultiQuery {
                index: i,
                query: format!("multi {i}"),
                tools: vec!["NewsTool".into(), "FinanceTool".into()],
            })
            .collect();
        let spec = SampleSpec {
            total: 20,
            multi_tool_ratio: 0.2,
            seed: 7,
        };
        let (a, _) = build_scenarios(&single, &multi, &plugins, &spec);
        let (b, _) = build_scenarios(&single, &multi, &plugins, &spec);
        let a_ids: Vec<_> = a.iter().map(|s| &s.id).collect();
        let b_ids: Vec<_> = b.iter().map(|s| &s.id).collect();
        assert_eq!(a_ids, b_ids);
    }

    #[test]
    fn build_scenarios_respects_total_and_multi_ratio() {
        let plugins = fixture_plugins();
        let single: Vec<RawSingleQuery> = (0..50)
            .map(|i| RawSingleQuery {
                line: i + 2,
            ...
            .collect();
        let multi: Vec<RawMultiQuery> = (0..50)
            .map(|i| RawMultiQuery {
                index: i,
                query: format!("m{i}"),
                tools: vec!["NewsTool".into()],
            })
            .collect();
        let spec = SampleSpec {
            total: 10,
            multi_tool_ratio: 0.3,
            seed: 1,
        };
        let (out, stats) = build_scenarios(&single, &multi, &plugins, &spec);
        assert_eq!(out.len(), 10);
        assert_eq!(stats.scenarios_out, 10);
        let multi_count = out
            .iter()
            .filter(|s| s.id.starts_with("metatool-mt-"))
            .count();
        // 10 * 0.3 = 3 multi exactly.
        assert_eq!(multi_count, 3);
    }

    #[test]
    fn build_scenarios_caps_when_corpus_smaller_than_target() {
        let plugins = fixture_plugins();
        let single: Vec<RawSingleQuery> = (0..3)
            .map(|i| RawSingleQuery {
                line: i + 2,
                query: format!("q{i}"),
                tool: "WeatherTool".into(),
            ...
            .collect();
        let multi: Vec<RawMultiQuery> = vec![RawMultiQuery {
            index: 0,
            query: "m".into(),
            tools: vec!["NewsTool".into()],
        }];
        let spec = SampleSpec {
            total: 100,
            multi_tool_ratio: 0.5,
            seed: 1,
        };
        let (out, stats) = build_scenarios(&single, &multi, &plugins, &spec);
        // Caps at 3 single + 1 multi = 4 even though target was 100.
        assert_eq!(out.len(), 4);
        assert_eq!(stats.scenarios_out, 4);
    }
    ...
    fn build_scenarios_emits_stable_id_sorted_output() {
        ...
        let (out, _) = build_scenarios(&single, &[], &plugins, &spec);
