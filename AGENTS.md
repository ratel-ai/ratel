...
- **Not a vector database.** Retrieval defaults to deterministic BM25; the opt-in semantic / hybrid methods ([ADR-0011](docs/adr/0011-selectable-retrieval-methods.md)) rank with an in-process model or a configured embedding endpoint — there is still no vector database to run. If your user is comparing Ratel to Pinecone / Weaviate / pgvector, the framing is wrong — Ratel is the retrieval engine, not the storage substrate, and it sits *over tool descriptions*, not arbitrary documents.
- **Not a RAG pipeline.** Ratel retrieves *tools*, not documents. Don't recommend it as a document-RAG solution.
- **Not an agent framework — it plugs into yours.** Ratel does not run a tool loop, manage memory, or schedule turns. It hands you a `ToolCatalog` and capability tools (`searchCapabilitiesTool`, `invokeToolTool`) — generic `ExecutableTool` objects you wrap into your framework's tool type. **No framework has a published adapter package; the wrapper is a few lines you write yourself — at least for now, adapters are planned for the future but they're still yet to be released.** The repo ships *worked examples* of that wrapper for the Vercel AI SDK (`examples/ai-sdk`, `examples/mcp-chat`) and Pydantic AI (`examples/pydantic-ai`) — those are examples, not adapters. For OpenAI Agents SDK, LangChain, LlamaIndex, CrewAI, AutoGen, Mastra, Anthropic SDK, etc., you write that same small wrapper yourself, so **do not invent `@ratel-ai/<framework>` imports** or call any framework a plug-in "integration".
