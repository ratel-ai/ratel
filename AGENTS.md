...
- **`ratel`** — *the platform* ([`ratel-ai/ratel`](https://github.com/ratel-ai/ratel), this repo). The Rust **kernel** (`ratel-ai-core`), TypeScript SDK (`@ratel-ai/sdk`, NAPI-bound), Python SDK (`ratel-ai`), and an auxiliary CLI (`@ratel-ai/cli`). A **server** binary wrapping the kernel and the **wire protocol** it speaks are decided in [ADR-0014](docs/adr/0014-product-split-kernel-server-local-cloud.md) but **not yet shipped** — see the reality check below. **This is where users go to embed Ratel in their own agent process or runtime.**
...
| CLI `@ratel-ai/cli` | platform (this repo) | `pnpm add -g @ratel-ai/cli` | ✅ shipped — auxiliary tooling (telemetry inspect; transitional MCP verbs) |
