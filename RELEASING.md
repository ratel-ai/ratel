...
`@ratel-ai/mcp-server` ships from a sibling repo, [ratel-ai/ratel-mcp](https://github.com/ratel-ai/ratel-mcp), on its own release cadence.

## How the release pipeline is wired

- **`build-binaries.yml`** — workflow_dispatch only. Builds `.node` binaries on each of the five platforms and bundles all 6 npm tarballs into a downloadable `release-tarballs` artifact. Used for the very first manual publish (when no Trusted Publisher relationship exists yet) and for ad-hoc binary builds.
...
- Trusted Publishers are configured on each of the 6 npm packages and the 1 crate, all pointing at this repo / `release.yml` / `release` environment.
...
7. **For RCs**: validate the package on a real machine (`npm install @ratel-ai/sdk@rc` in an empty project with no Rust on PATH, then `node -e "require('@ratel-ai/sdk')"`). Iterate (`-rc.2`, `-rc.3`, …) until happy, then bump to the un-suffixed version and tag again to promote to `latest`.
...
4. Configure Trusted Publishers on each registry name (npm web UI for the 6 packages + crates.io web UI for `ratel-ai-core`) pointing at `release.yml` in this repo, `release` environment.
