...
- **`workspace:^` rewriting** must be done by `pnpm pack` / `pnpm publish` — `npm publish` ships the literal string `"workspace:^"` and breaks installs (`EUNSUPPORTEDPROTOCOL` on `npm install`). The release workflow packs `mcp-server` and `cli` with `pnpm pack` first and then publishes the resulting tarball with `npm publish` (so OIDC + provenance still work). Don't pin internal deps to exact versions in source — that breaks patch-version uptake without a re-release.
