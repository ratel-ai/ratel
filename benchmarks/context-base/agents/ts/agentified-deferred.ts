...
import { MODEL, MAX_STEPS } from "../../lib/constants.js";
...
async function boot(): Promise<BootResult> {
  ...
  if (!endpoint) {
    console.error("[agentified-deferred] starting agentified-core container...");
    ...
    container = started;
    console.error(`[agentified-deferred] agentified-core running at ${endpoint}`);
  } else {
    console.error(`[agentified-deferred] using external endpoint: ${endpoint}`);
  }
  ...
  console.error(`[agentified-deferred] using search strategy: ${strategy}`);

  // All tools deferred by default (no alwaysInclude)
  const instance = await ag.register({ tools: backendTools });
  console.error(`[agentified-deferred] registered ${backendTools.length} tools (all deferred)`);
...
if (process.argv[1]?.endsWith("agentified-deferred.ts") || process.argv[1]?.endsWith("agentified-deferred.js")) {
  ...
  process.on("SIGTERM", async () => {
    ...
    if (result.container) {
      console.error("[agentified-deferred] stopping container...");
  ...
  startAgent({
    ...
    sendMessage: async (body: SendMessageBody): Promise<SendMessageResponse> => {
      ...
      // Start with discover tool only — all other tools are deferred
      const discoveredNames = discoverTool.discoveredNames;

      const result = await runAgenticLoop({
        client,
        model,
        system: DEFERRED_SYSTEM_PROMPT,
      ...
      return {
        ...
        turnId: body.turnId,
        debug: {
          systemPrompt: DEFERRED_SYSTEM_PROMPT,
